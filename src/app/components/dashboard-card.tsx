...
export default function DashboardCard({ card, className }: Props) {
  return (
    <div
      className={`${className} block lg:m-3 m-1 w-56 h-56 ${card.bgColor} rounded-2xl shadow-lg relative hover:scale-105 transition ease-in`}
    >
      <h1 className="ml-5 mt-5 text-xl font-extrabold">{card.header}</h1>
      <p className="mx-5">{card.subHeader}</p>
